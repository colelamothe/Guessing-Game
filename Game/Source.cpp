...
int main() {
	...
	int guessCount = 1; //game starts with first guess to allow increment to work properly
	...
	do { //overall shell to allow the game to be played multiple times on player input
		...
		cin >> guess; // receives first guess
	...
	} while (cont == 'y'); //allows the game to loop if the player inputs the character 'y' and exits otherwise
